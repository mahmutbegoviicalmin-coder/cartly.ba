...
const IMAGES = [
  { src: "/radne-patike-hammer/hero.webp", alt: "Hammer S3 - hero" },
  { src: "/radne-patike-hammer/main-image-1.webp", alt: "Hammer S3 - pogled 1" },
  { src: "/radne-patike-hammer/main-image-3.webp", alt: "Hammer S3 - pogled 2" },
  { src: "/radne-patike-hammer/main-image-4.webp", alt: "Hammer S3 - pogled 3" },
  { src: "/radne-patike-hammer/main-image-5.webp", alt: "Hammer S3 - pogled 4" },
  { src: "/radne-patike-hammer/main-image-6.webp", alt: "Hammer S3 - pogled 5" },
];
...
const FAQS = [
  ...
  { q: "Koje brojeve imate?",               a: "Dostupni su brojevi od 39 do 47 EU. Preporučujemo naručiti vaš uobičajeni broj, a ako ste između — odaberite veći." },
  ...
  { q: "Koja je garancija?",                a: "Hammer S3 patike dolaze s garancijom od 1 godine na materijal i izradu." },
...
function Hero({ onOrder }: { onOrder: () => void }) {
  ...
  const handleCTA = () => {
    fireFbq("track", "AddToCart", { content_name: "Hammer S3 Radne Patike", value: PRICE, currency: "BAM" });
    onOrder();
  };
          ...
          <div className="r-hero-grid" style={{ display: "flex", gap: 52, alignItems: "flex-start" }}>
            ...
            <div className="r-info-col r-info-inner" style={{ flex: 1, display: "flex", flexDirection: "column", gap: 0, paddingTop: 4 }}>
              ...
              <h1 style={{ fontSize: "clamp(32px,4.5vw,52px)", fontWeight: 900, lineHeight: 1.06, letterSpacing: "-0.03em", color: "#0A0A0A", marginBottom: 16, fontFamily: F }}>
                Radne Patike S3<br />
                <span style={{ color: "rgba(10,10,10,0.3)", fontWeight: 700 }}>Hammer</span>
              </h1>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
                {([
                  ...
                  { title: "Veličine 39–47", sub: "Dostupne sve veličine",
    ...
    <footer style={{ background: "#0A0A0A", padding: "48px 24px 36px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", fontFamily: F, margin: 0 }}>Hammer S3 Radne Patike · Plaćanje pouzećem · BiH</p>
        </div>
...
function OrderModal({ open, onClose, initialSize }: { open: boolean; onClose: () => void; initialSize?: number }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res  = await fetch("/api/hammer-order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...fields, velicina: size, kolicina: qty }) });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || "Greška");
      fireFbq("track", "Purchase", { value: total, currency: "BAM", content_name: "Hammer S3 Radne Patike", content_ids: ["radne-patike-hammer"], num_items: qty }, { eventID: data.orderNumber });
      setDone(true);
    } catch (err) {
        ...
        {done ? (
          ...
          <div style={{ padding: "24px 24px 48px" }}>
            ...
            <div style={{ marginBottom: 22 }}>
              <h2 style={{ fontSize: 20, fontWeight: 900, color: "#0A0A0A", letterSpacing: "-0.03em", margin: "0 0 4px", fontFamily: F }}>Naruči Hammer S3</h2>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
...
function QuickOrderSection({ onDone }: { onDone?: (orderNumber: string, total: number, qty: number) => void }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res  = await fetch("/api/hammer-order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ime: name, telefon: phone, adresa: addr, grad: city, velicina: velicine[0]?.velicina, kolicina: totalPairs, velicine }) });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || "Greška");
      fireFbq("track", "Purchase", { value: total, currency: "BAM", content_name: "Hammer S3 Radne Patike", content_ids: ["radne-patike-hammer"], num_items: totalPairs }, { eventID: data.orderNumber });
...
export default function HammerPage() {
  ...
  useEffect(() => {
    fireFbq("track", "ViewContent", { content_name: "Hammer S3 Radne Patike", content_ids: ["radne-patike-hammer"], content_type: "product", value: PRICE, currency: "BAM" });
  }, []);

  function openOrder(size?: number) {
    setInitialSize(size);
    setModalOpen(true);
    fireFbq("track", "InitiateCheckout", { content_name: "Hammer S3 Radne Patike", content_ids: ["radne-patike-hammer"], value: PRICE, currency: "BAM" });
  }
