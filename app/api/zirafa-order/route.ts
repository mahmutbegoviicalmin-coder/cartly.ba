...
export async function POST(request: NextRequest) {
  try {
    ...
    const { ime, telefon, adresa, grad, napomena, externalId } = body;
    ...
    const { error: dbError } = await getSupabaseAdmin()
      ...
      .insert({
        ...
        adresa,
        grad,
        velicine:        [{ velicina: "Žirafa Brusilica za Zidove", kolicina: 1 }],
        ukupno_pari:     1,
        cijena_proizvoda,
        dostava:         DELIVERY,
        ukupno,
    ...
    sendCAPIEvent({
      ...
      value:       ukupno,
      currency:    "BAM",
      contentName: "Žirafa Brusilica za Zidove",
      phone:       telefon,
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: `#NARUDZBA [ZIRAFA] - ${ime} - 1kom - ${sarajevoTime(now)}`,
        html: `
<div style="font-family:Inter,-apple-system,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #F0F0F0;">
  <div style="background:#0284C7;padding:32px 40px;">
  ...
  <div style="padding:32px 40px;">
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:7px 0;color:#888;font-size:14px;width:140px;">Proizvod</td><td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">Žirafa Brusilica za Zidove</td></tr>
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Cijena</td><td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">${fmtKM(cijena_proizvoda)} <span style="color:#bbb;text-decoration:line-through;">269,90 KM</span></td></tr>
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Dostava</td><td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">${fmtKM(DELIVERY)} · Euro Express · 1-2 dana</td></tr>
      <tr style="border-top:2px solid #F0F0F0;">
        <td style="padding:14px 0 0;color:#0A0A0A;font-size:15px;font-weight:700;">Ukupno</td>
        <td style="padding:14px 0 0;color:#0284C7;font-size:20px;font-weight:800;">${fmtKM(ukupno)}</td>
      </tr>
    </table>
  </div>
  <div style="background:#F9F9F9;padding:20px 40px;border-top:1px solid #F0F0F0;">
    <p style="font-size:12px;color:#aaa;margin:0;">Plaćanje pouzećem · Euro Express 1-2 dana</p>
    <p style="font-size:12px;color:#aaa;margin:8px 0 0;"><a href="tel:${telefon}" style="color:#0284C7;text-decoration:none;font-weight:600;">Pozovi kupca</a> &nbsp;·&nbsp; <a href="https://cartlyba.vercel.app/admin/dashboard" style="color:#0284C7;text-decoration:none;font-weight:600;">Otvori admin panel</a></p>
  </div>
</div>`,
      });
    } catch (emailErr) {
    ...
    console.error("Zirafa order route error:", err);
    return NextResponse.json(
