  ...
  try {
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: `#NARUDZBA [ZVUCNIK ZQS] - ${ime} - ${qty}kom - ${sarajevoTime(now)}`,
        html: `
<div style="font-family:Inter,-apple-system,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #F0F0F0;">
  ...
  <div style="padding:32px 40px;">
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:7px 0;color:#888;font-size:14px;width:140px;">Proizvod</td><td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">Bluetooth Zvučnik ZQS-6239</td></tr>
      ...
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Dostava</td><td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">${fmtKM(DELIVERY)} · Brza Pošta</td></tr>
  ...
  <div style="background:#F9F9F9;padding:20px 40px;border-top:1px solid #F0F0F0;">
    <p style="font-size:12px;color:#aaa;margin:0;">Plaćanje pouzećem · Brza Pošta · 1–3 radna dana</p>
  </div>
</div>`,
      });
    } catch (emailErr) {
    ...
    console.error("order-zvucnik route error:", err);
    return NextResponse.json(
