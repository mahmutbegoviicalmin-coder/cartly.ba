...
const SIZE_LABELS: Record<string, string> = {
  S: "S — Djeca do 9 godina",
  M: "M — Djeca do 15 godina",
  "3XL": "3XL — Odrasli",
  PORODICNI: "Porodični paket (S + M + 2XL + 2XL)",
};
const DELIVERY = 10.0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cijena_proizvoda = SIZE_PRICES[velicina];
    const ukupno = cijena_proizvoda + DELIVERY;
    const now = new Date();
    const orderNumber = generateOrderNumber(now);
    const bojaLabel = boja || "—";
    ...
    sendCAPIEvent({
      ...
      value: ukupno,
      currency: "BAM",
      contentName: "Sigurnosni Prsluk za Spašavanje",
      phone: telefon,
          ...
          <div style="font-family: Inter, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; background: #fff; border: 1px solid #F0F0F0;">
            ...
            <div style="padding: 32px 40px;">
              ...
              <table style="width:100%; border-collapse:collapse;">
                <tr><td style="padding:7px 0; color:#888; font-size:14px; width:140px;">Proizvod</td><td style="padding:7px 0; font-weight:600; font-size:14px; color:#0A0A0A;">Sigurnosni Prsluk za Spašavanje</td></tr>
                <tr><td style="padding:7px 0; color:#888; font-size:14px;">Veličina</td><td style="padding:7px 0; font-weight:600; font-size:14px; color:#0A0A0A;">${SIZE_LABELS[velicina]}</td></tr>
                <tr><td style="padding:7px 0; color:#888; font-size:14px;">Boja</td><td style="padding:7px 0; font-weight:600; font-size:14px; color:#0A0A0A;">${bojaLabel}</td></tr>
                ...
                <tr style="border-top:2px solid #F0F0F0;">
                  ...
                  <td style="padding:14px 0 0; color:#061B38; font-size:20px; font-weight:800;">${fmtKM(ukupno)}</td>
                </tr>
              </table>
            </div>
            <div style="background:#F9F9F9; padding:20px 40px; border-top:1px solid #F0F0F0;">
              <p style="font-size:12px; color:#aaa; margin:0;">Plaćanje pouzećem · Dostava 1–3 radna dana</p>
            </div>
          </div>
        `,
      });
    ...
    console.error("Prsluk order route error:", err);
