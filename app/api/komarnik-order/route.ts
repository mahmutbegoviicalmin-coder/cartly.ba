...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    sendCAPIEvent({
      ...
      value:       ukupno,
      currency:    "BAM",
      contentName: "Magnetni Komarnik za Vrata",
      phone:       telefon,
    ...
    await resend.emails.send({
      ...
      subject: `#NARUDZBA [KOMARNIK] · ${ime} · ${bundleInfo.label} · ${sarajevoTime(now)}`,
      html: `
<!DOCTYPE html>
<html lang="bs">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <td style="background:${ACCENT};border-radius:12px 12px 0 0;padding:28px 32px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
          <td>
            ...
            <h1 style="margin:0 0 4px;font-size:22px;font-weight:800;color:#fff;">Magnetni Komarnik za Vrata</h1>
            <p style="margin:0;font-size:13px;color:rgba(255,255,255,0.8);">${bundleInfo.label} · Samolepljivi okvir · Magnetno zatvaranje</p>
          </td>
    ...
    <tr>
      <td style="background:#fff;padding:32px;border-left:1px solid #E5E5E5;border-right:1px solid #E5E5E5;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #F0F0F0;border-radius:8px;overflow:hidden;margin-bottom:28px;">
          ...
          <tr>
            <td style="padding:12px 16px;font-size:14px;color:#0A0A0A;border-bottom:1px solid #F0F0F0;">Magnetni komarnik za vrata · ${bundleInfo.label}</td>
        ...
        <table cellpadding="0" cellspacing="0"><tr>
          ...
          <td>
            <a href="https://cartlyba.vercel.app/admin/dashboard" style="display:inline-block;padding:12px 22px;background:${ACCENT};color:#fff;text-decoration:none;font-size:14px;font-weight:600;border-radius:8px;">
              Potvrdi narudžbu
            </a>
          </td>
        </tr></table>
    ...
    <tr>
      <td style="background:#F9F9F9;border:1px solid #E5E5E5;border-top:none;border-radius:0 0 12px 12px;padding:20px 32px;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#0A0A0A;">Cartly.ba · Magnetni Komarnik za Vrata</p>
    ...
    console.error("Komarnik order error:", error);
    return Response.json({ success: false, error: "Greška pri slanju narudžbe." }, { status: 500 });
  }
}
