...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    sendCAPIEvent({
      ...
      value:       ukupno,
      currency:    "BAM",
      contentName: "Čelična Četka za Trimer",
      phone:       telefon,
    ...
    await resend.emails.send({
      ...
      subject: `#NARUDZBA [ČETKA] - ${ime} - ${setLabel} - ${sarajevoTime(now)}`,
      html: `
<!DOCTYPE html>
<html lang="bs">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
          ...
          <tr><td style="padding-bottom:8px;">
            <div style="background:#F7F7F7;border-radius:8px;padding:10px 14px;">
              <p style="margin:0 0 3px;font-size:10px;font-weight:700;color:#AAAAAA;text-transform:uppercase;letter-spacing:0.08em;">${row.label}</p>
              <p style="margin:0;font-size:16px;font-weight:700;color:#0A0A0A;">${row.value}</p>
            </div>
          </td></tr>`).join("")}
        </table>
    ...
    <tr>
      <td style="background:#F9F9F9;border:1px solid #E5E5E5;border-top:none;border-radius:0 0 12px 12px;padding:20px 32px;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#0A0A0A;">Cartly.ba &bull; Čelična Četka za Trimer</p>
    ...
    console.error("Cetka order error:", error);
