  ...
  try {
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: `#NARUDZBA [DEWALT] - ${ime} - 1kom - ${sarajevoTime(now)}`,
        html: `
<div style="font-family:Inter,-apple-system,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #F0F0F0;">
  ...
  <!-- Header -->
  <div style="background:#FFCD00;padding:32px 40px;">
    <h1 style="color:#000;margin:0;font-size:22px;font-weight:800;letter-spacing:-0.02em;">Nova narudžba — DeWalt</h1>
    <p style="color:rgba(0,0,0,0.6);margin:6px 0 0;font-size:13px;">${orderNumber} &nbsp;·&nbsp; ${formatDateBosnian(now)}</p>
  </div>
  ...
  <div style="padding:32px 40px;">
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:7px 0;color:#888;font-size:14px;width:150px;">Proizvod</td>
          <td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">DeWalt 28V XR Bušilica/Odvijač</td></tr>
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Komplet</td>
          <td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">2× Baterija, Punjač, Set alata, Tvrdi kofer</td></tr>
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Cijena</td>
          <td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">${fmtKM(cijena_proizvoda)}</td></tr>
      <tr><td style="padding:7px 0;color:#888;font-size:14px;">Dostava</td>
          <td style="padding:7px 0;font-weight:600;font-size:14px;color:#0A0A0A;">${fmtKM(DELIVERY)} · Euro Express</td></tr>
      <tr style="border-top:2px solid #F0F0F0;">
        <td style="padding:14px 0 0;color:#0A0A0A;font-size:15px;font-weight:700;">Ukupno</td>
        <td style="padding:14px 0 0;color:#B8960C;font-size:20px;font-weight:800;">${fmtKM(ukupno)}</td>
      </tr>
    </table>
  </div>
...
`,
    ...
    console.error("DeWalt order route error:", err);
    return NextResponse.json(
