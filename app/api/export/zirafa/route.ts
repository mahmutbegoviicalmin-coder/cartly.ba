...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await getSupabaseAdmin()
      .from("orders")
      .select("ime, telefon, adresa, grad, ukupno, order_number, created_at")
      .like("order_number", "ZRF%")
    ...
    if (error) {
      console.error("[zirafa export] Supabase error:", error.message);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    if (!data || data.length === 0) {
      return NextResponse.json(
        { error: `Nema Žirafa narudžbi za ${dateParam}.` },
        { status: 404 }
      );
    }
    ...
    return new NextResponse(buf, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Zirafa_Brusilica_${dateParam}.xlsx"`,
        "Content-Length": String(buf.length),
        "Cache-Control": "no-store",
      },
    });
  } catch (err) {
    console.error("[zirafa export] Error:", err);
    return NextResponse.json(
