...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!orders || orders.length === 0) {
      return NextResponse.json({ error: `Nema narudžbi brusilica za ${dateParam}.` }, { status: 404 });
    }
    ...
    return new NextResponse(buf, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Brusilice_Postin_${dateParam}.xlsx"`,
        "Content-Length":      String(buf.length),
        "Cache-Control":       "no-store",
      },
    });
  } catch (err) {
    console.error("[brusilice export] Error:", err);
