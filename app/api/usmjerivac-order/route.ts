...
const BUNDLE_CONFIG: Record<number, { label: string; price: number; delivery: number }> = {
  1: { label: "1 komad",  price: 13.90, delivery: 10.00 },
  2: { label: "2 komada", price: 21.90, delivery: 10.00 },
  3: { label: "3 komada", price: 28.90, delivery: 10.00 },
  4: { label: "4 komada", price: 34.90, delivery: 10.00 },
};
const ACCENT   = "#2563EB";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { bundle = 1, externalId } = body as {
      bundle: number; externalId?: string;
    };

    const bundleInfo      = BUNDLE_CONFIG[bundle] ?? BUNDLE_CONFIG[1];
    const now             = new Date();
    const orderNumber     = generateOrderNumber(now);
    const dateBosnian     = formatDateBosnian(now);
    const cijenaProizvoda = bundleInfo.price;
    const dostava         = bundleInfo.delivery;
    ...
    const { error: dbError } = await getSupabaseAdmin()
      ...
      .insert({
        ime, telefon, adresa: stampIp(adresa, guarded.fields.ip), grad,
        bundle,
    ...
    sendCAPIEvent({
      ...
      value:       ukupno,
      currency:    "BAM",
      contentName: "Usmjerivač Zraka Klime",
      phone:       telefon,
    ...
    await resend.emails.send({
      ...
      subject: `#NARUDZBA [USMJERIVAC] · ${ime} · ${bundleInfo.label} · ${sarajevoTime(now)}`,
      html: `
<!DOCTYPE html>
<html lang="bs">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <td style="background:${ACCENT};border-radius:12px 12px 0 0;padding:28px 32px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
          <td>
            ...
            <h1 style="margin:0 0 4px;font-size:22px;font-weight:800;color:#fff;">Usmjerivač Zraka Klime</h1>
            <p style="margin:0;font-size:13px;color:rgba(255,255,255,0.8);">${bundleInfo.label} · Podesivi kut · Bez bušenja</p>
          </td>
    ...
    <tr>
      <td style="background:#fff;padding:32px;border-left:1px solid #E5E5E5;border-right:1px solid #E5E5E5;">
        <p style="margin:0 0 12px;font-size:11px;font-weight:700;color:${ACCENT};text-transform:uppercase;letter-spacing:0.1em;">Podaci kupca</p>
        ...
        <p style="margin:0 0 12px;font-size:11px;font-weight:700;color:${ACCENT};text-transform:uppercase;letter-spacing:0.1em;">Narudžba</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #F0F0F0;border-radius:8px;overflow:hidden;margin-bottom:28px;">
          ...
          <tr>
            <td style="padding:12px 16px;font-size:14px;color:#0A0A0A;border-bottom:1px solid #F0F0F0;">Usmjerivač zraka klime · ${bundleInfo.label}</td>
          ...
          <tr>
            ...
            <td style="padding:10px 16px;font-size:13px;color:#0A0A0A;font-weight:500;text-align:right;border-bottom:1px solid #F0F0F0;">${dostava === 0 ? "Besplatno" : fmt(dostava)}</td>
          </tr>
          <tr style="background:#EFF6FF;">
            <td style="padding:14px 16px;font-size:15px;font-weight:700;color:#0A0A0A;">UKUPNO</td>
            <td style="padding:14px 16px;font-size:20px;font-weight:800;color:${ACCENT};text-align:right;">${fmt(ukupno)}</td>
          </tr>
        </table>
        <p style="margin:0 0 14px;font-size:11px;font-weight:700;color:${ACCENT};text-transform:uppercase;letter-spacing:0.1em;">Akcija</p>
        <table cellpadding="0" cellspacing="0"><tr>
          ...
          <td>
            <a href="https://cartlyba.vercel.app/admin/dashboard" style="display:inline-block;padding:12px 22px;background:${ACCENT};color:#fff;text-decoration:none;font-size:14px;font-weight:600;border-radius:8px;">Potvrdi narudžbu</a>
          </td>
        </tr></table>
      </td>
    </tr>
    <tr>
      <td style="background:#F9F9F9;border:1px solid #E5E5E5;border-top:none;border-radius:0 0 12px 12px;padding:20px 32px;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#0A0A0A;">Cartly.ba · Usmjerivač Zraka Klime</p>
    ...
    console.error("Usmjerivac order error:", error);
    return Response.json({ success: false, error: "Greška pri slanju narudžbe." }, { status: 500 });
  }
}
