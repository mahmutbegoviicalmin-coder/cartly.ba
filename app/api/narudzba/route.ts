...
export async function POST(request: NextRequest) {
  try {
    ...
    sendCAPIEvent({
      ...
      value:       ukupno,
      currency:    "BAM",
      contentName: "Milwaukee M18 Bušilica Set",
      phone:       telefon,
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: `#NARUDZBA [BUSILICA M18] - ${ime} - 1kom - ${sarajevoTime(now)}`,
        html: `
          <div style="font-family: Inter, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #F0F0F0;">
            <div style="background: #E8460A; padding: 32px 40px;">
            ...
            <div style="padding: 32px 40px;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 7px 0; color: #888; font-size: 14px; width: 140px;">Proizvod</td><td style="padding: 7px 0; font-weight: 600; font-size: 14px; color: #0A0A0A;">Milwaukee M18 Bušilica</td></tr>
                <tr><td style="padding: 7px 0; color: #888; font-size: 14px;">Količina</td><td style="padding: 7px 0; font-weight: 600; font-size: 14px; color: #0A0A0A;">1×</td></tr>
                ...
                <tr style="border-top: 2px solid #F0F0F0;">
                  ...
                  <td style="padding: 14px 0 0; color: #E8460A; font-size: 20px; font-weight: 800;">${fmtKM(UNIT_PRICE + DELIVERY)}</td>
                </tr>
              </table>
            </div>
            <div style="background: #F9F9F9; padding: 20px 40px; border-top: 1px solid #F0F0F0;">
              <p style="font-size: 12px; color: #aaa; margin: 0;">Plaćanje pouzećem · Besplatna dostava · 1–2 radna dana</p>
            </div>
          </div>
        `,
      });
      ...
      // Ne blokiramo narudžbu ako email ne stigne
    }

    return NextResponse.json({ success: true, orderNumber });
  } catch (err) {
    console.error("Milwaukee order route error:", err);
    return NextResponse.json(
